...
import PageInput from './PageInput.poc';
...
export default function AppPoc() {
  ...
  const loadSavedData = () => {
    try {
      ...
      if (savedData) {
        ...
        const pagesWithIds = parsed.pages.map((page) => ({ ...page, id: page.id || nanoid() }));
        return {
          pages: pagesWithIds || [{
            id: nanoid(),
            title: "Welcome!",
  ...
  const handleAddPage = () => {
    setPages([...pages, { id: nanoid(), title: "", content: "", backgroundColor: "#e0f2fe", image: null, template: "simple" }]);
  };
